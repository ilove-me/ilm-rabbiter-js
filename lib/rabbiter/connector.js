      ...
      .then(function () {
        ...
        var camelCased = function(hash){
          _.map(hash, function(val, key) {
            ...
            if(key){
              delete hash[key];
              hash[ _.camelCase(key)] = val;
            }

          });
        };

        var messageContent = JSON.parse(rabbitMsg.content);
        camelCased(messageContent);

        //execute callback and send response if there is a queue and id to respond to
        return PromiseBlue.resolve(callbackFunction(messageContent))
          .then(function(msg){
